...
<div class="col-md-6 " style="margin-left: 30%">
 ...
 @if ($message = Session::get('success'))
                ...
                <form class="form-horizontal form-label-left" action="{{route('adminservices.update',$service[0]->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                ...
                <div class="col-md-9 col-sm-9 ">
                    <input type="text" class="form-control" placeholder="Course's name in Arabic" name="name_en" value="{{ $service[0]->name_en }}">
                </div>
            </div>
            @error('name_ar')
            <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
            @enderror
                  ...
                  <div class="form-group row ">
                    ...
                    id='body_ar'
                    rows='10' cols='10' labelCol='2'>{{ $service[0]->body_ar }}</textarea></div>
                </div>
                 </div>
                ...
                <div class="form-group row ">
                    ...
                    class='form-control  wysiwyg'
                    id='body_ar'
                    rows='10' cols='10' labelCol='2' >{{ $service[0]->body_en }}</textarea></div>
                </div>
										...
										<div class="form-group row ">
											...
											<div class="col-md-9 col-sm-9 ">
												<input type="file" name="image" value="{{ $service[0]->image }}" id="image"> <label for="fileupload"> Select a file to upload</label> <br>
									</div>
									<img src="images/{{ $service[0]->image }}" style="width:20%;height:30vh;margin-left:13vw">
										</div>
										@error('image')
										<div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
										@enderror
