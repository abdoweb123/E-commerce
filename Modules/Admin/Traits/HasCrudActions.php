...
trait HasCrudActions
{
    ...
    public function create()
    {
        ...
        return view("{$this->viewPath}.create", $data,compact('countries'));
    }
    ...
    public function store(Request $request)
    {
        ...
        // Store product_price for countries
        $product_id = $entity->meta->entity_id;

        $country_ids = $request->country_id;
        $product_country_prices = $request->product_country_price;

        try {
            for($i=0; $i<count($country_ids); $i++){

                $product_countries = DB::table('product_countries')
                    ->insertOrIgnore([
                        'product_id'=>$product_id,
                        'country_id'=>$country_ids[$i],
                        'product_price'=>$product_country_prices[$i],
                    ]);
            }
        }
